...
export class RpcGateway {
    ...
    async requestMachineClaudeOptions(
        ...
        return await this.machineRpc(machineId, 'set-machine-claude-options', {
            models,
    ...
    async spawnSession(
        ...
        try {
            const result = await this.machineRpc(
                machineId,
                'spawn-happy-session',
                { type: 'spawn-in-directory', directory, agent, model, modelReasoningEffort, yolo, sessionType, worktreeName, resumeSessionId, effort, permissionMode, allowedTools }
            )
